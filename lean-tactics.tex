...
the same expression.
For tactics not in the core, the necessary import is written in gray.
...
\begin{tabular}{@{}lll@{}}
 ...
 $\exists$ (there exists) & \makecell[lt]{\lean{use} \expr \\ \color{gray}\lean{import Mathlib.Tactic.Use}} & \makecell[lt]{\lean{cases'} \expr \lean{with} \nv \nv \\ \color{gray}\lean{import Mathlib.Tactic.Cases}} \\
 $\to$ (implies) & \lean{intro} \nv & \lean{apply} \expr or \lean{specialize} \nom \expr \\
 $\leftrightarrow$ (if and only if) & \lean{constructor}  & \lean{rw [}\expr\lean{]} or \lean{rw [←} \expr\lean{]}\\
 $\land$ (and) & \lean{constructor} & \makecell[lt]{\lean{cases'} \expr \lean{with} \nv \nv \\ \color{gray}\lean{import Mathlib.Tactic.Cases}} \\
 $\lor$ (or) & \makecell[lt]{\lean{left} or \lean{right} \\ \color{gray}\lean{import Mathlib.Tactic.LeftRight}} & \makecell[lt]{\lean{cases'} \expr \lean{with} \nv \nv \\ \color{gray}\lean{import Mathlib.Tactic.Cases}} \\
 $\lnot$ (not) & \lean{intro} \nv & \lean{apply} \expr or \lean{specialize} \nom \expr  \\
  \bottomrule
\end{tabular}
\end{center}
